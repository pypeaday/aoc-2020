    ...
    for _line in data:
        ...
        for _item in allowed_contents:
            ...
            # how many edge cases could there be
                ...
                raise SystemError(
                    "Fix this get_data function because apparently hundreds of bags are an option"
                )
...
def count_all_bag_capacities_including_self(rules: dict, sums: dict) -> dict:
    for bag, rule in rules.items():
        ...
        if rule[0] is None:
            ...
            if set(sums.keys()) != set() and set(rule).issubset(
                sums.keys()
            ):  # ie. if every bag in the rule has a numeric value already
